 ...
 *
 * The sampler wrapper module provides functionality for simple handling of
 ...
 *
 * The former follows the constructor format in OpenCL 1.0-1.2, where three
 * main sampler properties are directly given as constructor arguments, namely
 * the normalized coordinates flag, the addressing mode and the filter mode.
 * The later uses the OpenCL 2.0 constructor format, in which sampler
 * properties are given in a zero-terminated array of `cl_sampler_properties`.
 * Both formats can be used without concern for the underlying platform's
 * OpenCL version, because _cf4ocl_ will automatically select the most adequate
 * OpenCL constructor.
 *
 * Sampler wrapper objects should be freed with the ::ccl_sampler_destroy()
 * function, in accordance with the _cf4ocl_ @ref ug_new_destroy "new/destroy"
 * rule.
 *
 ...
 * ::ccl_kernel_set_args().
 *
 * Information about sampler objects can be fetched using the sampler
 ...
 * @return The requested sampler information object. This object will be
 * automatically freed when the sampler wrapper object is destroyed. If an
 ...
 *
 * @note As of OpenCL 2.0, there are no array sampler information values. This
 * macro is provided for completeness.
 *
