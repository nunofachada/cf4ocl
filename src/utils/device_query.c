...
int main(int argc, char ** argv) {
	...
	gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get number of platforms.", status);
	
	/* Get platform IDs. */
	platforms = (cl_platform_id*) malloc(numPlatforms * sizeof(cl_platform_id));
	gef_if_error_create_goto(err, CL4_ERROR, platforms == NULL, CL4_ERROR_NOALLOC, error_handler, "Unable to allocate memory for list of platform IDs.");

	status = clGetPlatformIDs(numPlatforms, platforms, NULL);
	gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get list of platform IDs.", status);
	...
	for(unsigned int i = 0; i < numPlatforms; i++) {
		...
		gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get platform vendor.", status);
		...
		gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to platform devices.", status);
		...
		for (unsigned int j = 0; j < numDevices; j++) {
			...
			gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get device name.", status);

			printf("\tDevice #%d: %s\n", j, pbuff);
			
			/* Device vendor. */
			status = clGetDeviceInfo(devices[j], CL_DEVICE_VENDOR, sizeof(pbuff), pbuff, NULL);
			gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get device vendor.", status);

			printf("\t           Vendor: %s\n", pbuff);

			/* Device type. */
			status = clGetDeviceInfo(devices[j], CL_DEVICE_TYPE, sizeof(cl_device_type), &dtypeaux, NULL);
			gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get device type.", status);
			...
			gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get device OpenCL C version.", status);
			
			printf("\t           %s\n", pbuff);
			
			/* Max. compute units. */
			status = clGetDeviceInfo(devices[j], CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(uintaux), &uintaux, NULL);
			gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get device max. compute units.", status);

			printf("\t           Max. Compute units: %d\n", uintaux);

			/* Global memory info. */
			status = clGetDeviceInfo(devices[j], CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(ulongaux), &ulongaux, NULL);
			gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get device global memory size.", status);

			status = clGetDeviceInfo(devices[j], CL_DEVICE_MAX_MEM_ALLOC_SIZE, sizeof(ulongaux2), &ulongaux2, NULL);
			gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get device maximum allocable memory.", status);

			status = clGetDeviceInfo(devices[j], CL_DEVICE_HOST_UNIFIED_MEMORY, sizeof(boolaux), &boolaux, NULL);
			gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get determine if device memory is unified with host.", status);
			...
			gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get type of local memory in device.", status);

			status = clGetDeviceInfo(devices[j], CL_DEVICE_LOCAL_MEM_SIZE, sizeof(ulongaux), &ulongaux, NULL);
			gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get size of local memory in device.", status);
			...
			if (argc > 1) {
				...
				gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get maximum constant buffer size.", status);
				...
				gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get device endianess.", status);
				...
				gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get preferred vector width for char.", status);

				printf(" Char=%d,", uintaux);

				status = clGetDeviceInfo(devices[j], CL_DEVICE_PREFERRED_VECTOR_WIDTH_SHORT, sizeof(uintaux), &uintaux, NULL);
				gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get preferred vector width for short.", status);

				printf(" Short=%d,", uintaux);

				status = clGetDeviceInfo(devices[j], CL_DEVICE_PREFERRED_VECTOR_WIDTH_INT, sizeof(uintaux), &uintaux, NULL);
				gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get preferred vector width for int.", status);

				printf(" Int=%d,", uintaux);

				status = clGetDeviceInfo(devices[j], CL_DEVICE_PREFERRED_VECTOR_WIDTH_LONG, sizeof(uintaux), &uintaux, NULL);
				gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get preferred vector width for long.", status);

				printf(" Long=%d,", uintaux);

				status = clGetDeviceInfo(devices[j], CL_DEVICE_PREFERRED_VECTOR_WIDTH_FLOAT, sizeof(uintaux), &uintaux, NULL);
				gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get preferred vector width for float.", status);
				...
				gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get preferred vector width for double.", status);
				...
				gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get preferred vector width for half.", status);

				printf(" Half=%d.\n", uintaux);
				
				/* Acceptable command queue properties. */
				status = clGetDeviceInfo(devices[j], CL_DEVICE_QUEUE_PROPERTIES, sizeof(cqpaux), &cqpaux, NULL);
				gef_if_error_create_goto(err, CL4_ERROR, CL_SUCCESS != status, status, error_handler, "OpenCL error %d: unable to get acceptable command queue properties.", status);
