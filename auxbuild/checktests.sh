...
#
# This script has been superseded by better code coverage alternatives.
#
...
do
    ...
    do
        callers=$(cflow $test_src | \
            awk '/^    [^ ]/ { caller=$1 }; /^.*'$ccl_fun'/ { printf caller " " }')
...
printf "\n SUMMARY: cf4ocl has ${#fun_table[@]} functions, \
    $untested_funs of which are untested.\n"
